      ...
      <div>
         <ul  className={css.castList}>
          {Array.isArray(cast) &&
                  cast.map((actor) => (
                <li key={actor.id}>
                    <img src={actor.profile_path ? `https://image.tmdb.org/t/p/w200${actor.profile_path}`: defaultImg} alt={actor.name} />
                    <p>{actor.name}</p>
                    <p><b>Character:</b> <br /> {actor.character ? actor.character : 'not found'}</p>
              </li>
            ))}
        </ul> 
    </div>
